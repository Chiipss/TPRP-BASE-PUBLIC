			...
			for i = 1, #Config.VehiclePositions, 1 do
				if Config.VehiclePositions[i]["entityId"] ~= nil then
					...
					if dstCheck <= 2.0 then
						sleepThread = 5
						DrawText3Ds(vehCoords.x, vehCoords.y, vehCoords.z + 1.0, "[~g~E~w~] ".. Config.VehiclePositions[i]["price"] ..":-")
						if IsControlJustPressed(0, 38) then
...
function OpenSellMenu(veh, price, buyVehicle, owner)
	...
	function(data, menu)
		...
		elseif action == "sell" then
			...
			ESX.TriggerServerCallback("esx_lg_vehiclesale:isVehicleValid", function(valid)
				if valid == 'yes' then
					DeleteVehicle(veh)
