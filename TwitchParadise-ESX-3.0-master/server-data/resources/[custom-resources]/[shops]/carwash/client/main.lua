	...
	while true do
		...
		if CanWashVehicle() then
			...
			for i=1, #Config.Locations, 1 do
				...
				if distance < 25 then
					DrawMarker(27, carWashLocation + 0.97, 0, 0, 0, 0, 0, 0, 5.0, 5.0, 2.0, 255, 255, 255, 50, false, false, 2, false, false, false, false)
					canSleep = false
				end
				...
				if distance < 5 then
					...
					if IsControlJustReleased(0, Keys['E']) then
						...
						if GetVehicleDirtLevel(vehicle) > 2 then
							...
							exports['mythic_notify']:SendAlert('error', 'Your vehicle is allready clean.')
	...
	ESX.TriggerServerCallback('esx_carwash:canAfford', function(canAfford)
		if canAfford then
			...
			if Config.EnablePrice then
				...
				exports['mythic_notify']:SendAlert('success', 'Your vehicle has been washed')
				--ESX.ShowNotification(_U('wash_successful'))
			end
			Citizen.Wait(5000)
		else
			exports['mythic_notify']:SendAlert('success', 'You can not afford a car wash')
