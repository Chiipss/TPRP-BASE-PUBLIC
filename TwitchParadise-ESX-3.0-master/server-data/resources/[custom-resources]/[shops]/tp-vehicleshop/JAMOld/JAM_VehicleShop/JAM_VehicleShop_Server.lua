...
ESX.RegisterServerCallback('JAM_VehicleShop:PurchaseVehicle', function(source, cb, model, price)
	...
	local plyMon = xPlayer.getMoney()
		...
		MySQL.Async.fetchAll('SELECT price FROM vehicles WHERE model = @model', {["@model"] = model}, function(result)
			if result[1] ~= nil and result[1]["price"] ~= nil then
				if result[1]["price"] ~= newPrice then
					print("ERROR: Given Price Did Not Match DB Price - Applying Ban")
					-- BAN MOTHERFUCKERS
					TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'I would appriciate it if you wouldnt cheat....'})
					-- I WANT THEM TO SEE WE CAUGHT THEM...
					Citizen.Wait(1000)
					TriggerEvent('banCheater', source, "Well what a fuckwit you are..... 😈 .. Byeeeee")
					TriggerClientEvent('LG_CLBan', source)
					TriggerEvent('DiscordBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n' .. GetPlayerName(source) .. '\n```', SystemAvatar, false)
					hasenough = false -- Stop the vehicle selling entirely.
				else
				...
				print("ERROR: Vehicleshop Did Not Return a Price From Database")
			end
		end)
