...
ESX.RegisterServerCallback('JAM_VehicleFinance:CheckFunds', function(source, cb, financeAmount, lastVeh)	
	...
	if not JIS or not JIS.ShopData then return; end
...
ESX.RegisterServerCallback('JAM_VehicleFinance:RepoVehicleEnd', function(source, cb, vehicle)
	...
	if canDel then
		...
		local carCost = 0
		for k,v in pairs(JIS.ShopData) do for k,v in pairs(v) do if (GetHashKey(v.model) % 0x100000000) == model then carCost = v.price; end; end; end
