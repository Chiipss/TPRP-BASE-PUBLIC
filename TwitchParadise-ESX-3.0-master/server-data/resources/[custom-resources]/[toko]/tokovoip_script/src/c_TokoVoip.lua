		...
		while (true) do
			...
			if (IsControlPressed(0, self.radioKey) and self.plugin_data.radioChannel ~= -1) then -- Talk on radio
				...
				if (not getPlayerData(self.serverId, "radio:talking")) then
					lastTalkProx = self.mode -- CAPTURE CURRENT PROXIMITY LEVEL
					self.mode = 2 -- Set Self to Whisper
					setPlayerData(self.serverId, "voip:mode", self.mode, true); -- Updates you to whisper mode
				...
				if lastTalkState == true then
					...
					setPlayerData(self.serverId, "voip:mode", self.mode, true); -- returns you to last state properly
				end
