...
RegisterNetEvent("tp:k9drugsniff")
AddEventHandler("tp:k9drugsniff", function()
    if DoesEntityExist(current_dog) then
		if not IsPedDeadOrDying(current_dog) then
			if GetDistanceBetweenCoords(GetEntityCoords(current_dog), GetEntityCoords(PlayerPedId()), true) <= 3.0 then
				local player, distance = ESX.Game.GetClosestPlayer()
				if distance ~= -1 then
					if distance <= 3.0 then
						local playerPed = GetPlayerPed(player)
						if not IsPedInAnyVehicle(playerPed, true) then
							TriggerServerEvent('tp-policedog:hasClosestDrugs', GetPlayerServerId(player))
						end
					end
				end
			end
...
RegisterNetEvent('tp-policedog:hasDrugs')
AddEventHandler('tp-policedog:hasDrugs', function(hadIt)
    if hadIt then
        TriggerServerEvent("tp:addChatSystem", "Your doggo has picked up a scent")
        loadDict('missfra0_chop_find')
        TaskPlayAnim(current_dog, 'missfra0_chop_find', 'chop_bark_at_ballas', 8.0, -8, -1, 0, 0, false, false, false)
    else
		--exports['mythic_notify']:SendAlert('inform', 'Doesnt find any drugs', 7000)
		TriggerServerEvent("tp:addChatSystem", "Your doggo doesn't find anything")
    end
end)
...
Citizen.CreateThread(function()
    ...
    if Config.Command then
        RegisterCommand(Config.Command, function()
            TriggerEvent('tp-policedog:openMenu')
        end)
    end
    while true do
        local sleep = 250
...
Citizen.CreateThread(function()
    while true do
        ...
        if current_dog == 0 then
            ...
            Citizen.Wait(1)
            if IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and not dogvehicled then
                TriggerEvent("K9:Vehicle")
            end
 
            if not IsPedSittingInAnyVehicle(GetPlayerPed(-1)) and dogvehicled then
                TriggerEvent("K9:Vehicle")
            end
        end
    end
...
RegisterNetEvent("K9:Vehicle")
AddEventHandler("K9:Vehicle", function()
    if IsPedInAnyVehicle(policeDog, false) and imDead == 0 and currentState == "F" then
        ClearPedTasks(policeDog)
        dogvehicled = false
        plyPos = GetEntityCoords(GetPlayerPed(-1),  true)
        local xnew = plyPos.x+4
        local ynew = plyPos.y+4
 
        SetEntityCoords(policeDog, xnew, ynew, plyPos.z)
        ...
        local vehicle = GetVehiclePedIsIn(GetPlayerPed(-1),false)
            for i=1,GetVehicleMaxNumberOfPassengers(vehicle) do
              if IsVehicleSeatFree(vehicle,i) then
                RequestAnimDict("creatures@rottweiler@in_vehicle@std_car")
                Citizen.Wait(100)
                TaskPlayAnim(policeDog, "creatures@rottweiler@in_vehicle@std_car", "sit", 1.0, -1, -1, 2, 0, 0, 0, 0)
                SetPedIntoVehicle(policeDog,vehicle,i)
                dogvehicled = true                     
                return true
              end
        end
...
RegisterNetEvent('tp-policedog:openMenu')
AddEventHandler('tp-policedog:openMenu', function()
    mainMenu()
end)
...
AddEventHandler('tp-policedog:hasDrugs', function(hadIt)
    if hadIt then
        exports['mythic_notify']:SendAlert('error', 'DRUGS FOUND!', 10000)
        loadDict('missfra0_chop_find')
...
mainMenu = function()
    if PlayerData.job.name == Config.Job then
            ...
            function(data, menu)
                if data.current.value == 'take_out_remove' then
                    if not DoesEntityExist(policeDog) then
                        ...
                        SetPedMaxHealth(policeDog, 9000)
                        SetEntityHealth(policeDog, 9000)
                        ...
                        exports['mythic_notify']:SendAlert('inform', 'Your partner in crime is here!', 7000)
                    else
                        exports['mythic_notify']:SendAlert('inform', 'Sleepy time for dogo', 7000)
                ...
                elseif data.current.value == 'attack_closest' then
                    if DoesEntityExist(policeDog) then
                        if not IsPedDeadOrDying(policeDog) then
                            ...
                            exports['mythic_notify']:SendAlert('error', 'Your dogo is dead!', 7000)
                        end
                    else
                        exports['mythic_notify']:SendAlert('inform', 'You dont have a dog', 7000)
                    end
                elseif data.current.value == 'sit_stand' then
                    if DoesEntityExist(policeDog) then
                        if GetDistanceBetweenCoords(GetEntityCoords(PlayerPedId()), GetEntityCoords(policeDog), true) <= 10.0 then
                            ...
                            exports['mythic_notify']:SendAlert('inform', 'Your dog is too far away', 7000)
                        end
                    else
                        exports['mythic_notify']:SendAlert('inform', 'You dont have a dog', 7000)
                    end
        ...
        exports['mythic_notify']:SendAlert('inform', 'You are not an officer', 7000)
    end
end
