local bossUI = {x = 568.47210693359, y = -3125.8894042969, z = 18.768611907959}
...
Citizen.CreateThread(function()
	while true do
        ...
        if(distance<2.5 and ESX.PlayerData.job.grade_name == "boss" and ESX.PlayerData.job.name == "merryweather") then
            sleepTime = 0
			DrawM('[E] Open Merryweather Menu', 27, bossUI.x, bossUI.y, bossUI.z - 0.945, 255, 255, 255, 1.5, 15)
			if(IsControlJustPressed(1, 38)) then
                renderMenu(name, "Merryweather Boss Menu")
			end
        end
...
function renderMenu(name, menuName)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'merryweathermenu',
		{
			title    = menuName,
			align    = 'bottom-right',
			elements = elements
