...
RegisterServerEvent("esx-ecobottles:retrieveBottleWeed")
AddEventHandler("esx-ecobottles:retrieveBottleWeed", function()
    local player = ESX.GetPlayerFromId(source)

    math.randomseed(os.time())
    local luck = math.random(0, 100)

    if luck >= 0 and luck <= 93 then
        --TriggerClientEvent("esx:showNotification", source, "The bin had no bottles in it.")
    else
        player.addInventoryItem("highgrademaleseed", 1)
        TriggerClientEvent("esx:showNotification", source, ("You found x%s Male weed seed"):format(1))
    end
end)
