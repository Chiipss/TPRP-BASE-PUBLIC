...
AddEventHandler('tostdostawa:rampanpcdawaj',function(a)
    ...
    if a=='1'then
        if zrespione==true then
            ESX.ShowNotification('~r~Complete the previous order!')
            return
        end
        local b=math.random(1,5)
        if b==1 then
            naklejkaAuto=3
            ESX.ShowNotification('~g~The driver awaits loading the boards.')
            ZrespPaczuszke('deski')
        elseif b==2 then
            naklejkaAuto=4
            ESX.ShowNotification('~g~The driver is waiting for the fish to be loaded.')
            ZrespPaczuszke('lody')
        elseif b==3 then
            naklejkaAuto=6
            ESX.ShowNotification('~g~The driver is waiting for the medicine to be loaded.')
            ZrespPaczuszke('leki')
        elseif b==4 then
            naklejkaAuto=2
            ESX.ShowNotification('~g~The driver awaits loading Logger Light drinks.')
            ZrespPaczuszke('napoje')
        elseif b==5 then
            naklejkaAuto=1
            ESX.ShowNotification('~g~The driver awaits loading the coffee.')
            ZrespPaczuszke('kawa')
        end
        ...
        ESX.ShowNotification('~y~The driver is on his way.')
        zrespione=true
        silnikOn=true
        Citizen.Wait(900)
        while silnikOn do
        ...
        ESX.ShowNotification('~r~The driver stopped and opened the trunk')
        ...
        ESX.ShowNotification('~r~Package loaded, driver wraps up.')
        ESX.ShowNotification('~w~You have loaded the package in ~b~'..czasikDostaw..' seconds.')
        if czasikDostaw<41 then
            ESX.ShowNotification('~b~Bonus ~g~'..premia1 ..'$~b~ for fast delivery')
            TriggerServerEvent("tostdostawa:wykonanieMisji",premia1)
            Citizen.Wait(200)
        elseif czasikDostaw>=41 and czasikDostaw<=55 then
            ESX.ShowNotification('~b~Bonus ~g~'..premia2 ..'$~b~ for fast delivery')
            TriggerServerEvent("tostdostawa:wykonanieMisji",premia2)
            Citizen.Wait(200)
        elseif czasikDostaw>=56 and czasikDostaw<=65 then
            ESX.ShowNotification('~b~Bonus ~g~'..premia3 ..'$~b~ for fast delivery')
            TriggerServerEvent("tostdostawa:wykonanieMisji",premia3)
            Citizen.Wait(200)
        elseif czasikDostaw>65 then
            ESX.ShowNotification('~w~No bonus for quick delivery')
        end
        czasikDostaw=0
        dostawaTimer=false
        ...
        ESX.ShowNotification('~r~You can accept the next order')
    elseif a=='2'then
        if zrespione==true then
            ESX.ShowNotification('~r~Complete the previous order!')
            return
        ...
        elseif b==3 then
            naklejkaAuto=6
            ESX.ShowNotification('~g~The driver is awaiting loading of medications.')
            ZrespPaczuszke('leki')
        elseif b==4 then
            naklejkaAuto=2
            ESX.ShowNotification('~g~The driver awaits loading Logger Light drinks.')
...
Citizen.CreateThread(function()
    while true do
		...
		czasikDostaw = czasikDostaw +1
			...
			ESX.ShowNotification('~r~The order was canceled because you took too long loading')
			end
		else
		Citizen.Wait(2000)
		end	
...
function PozyczPojazd(a)
    if a=='1'then
        if PojazdWyjety==0 then
            ...
            PojazdWyjety=0
            DeleteEntity(wozek)ESX.ShowNotification('~y~Your previous vehicle has been deleted, you can retreive a new one')
        end
    elseif a=='2'then
        if PojazdWyjety==0 then
            RequestModel(GetHashKey('forklift'))
...
Citizen.CreateThread(function()
	while true do 
		...
		if PrzejetyHangar==1 then 
			if b>205.0 then 
				ESX.ShowNotification('~y~Too far away from the warehouse because you lost the owner.')
