	...
	while true do
		...
		if IsEntityDead(PlayerPedId()) then
			...
			if DeathReason == 'committed suicide' or DeathReason == 'died' then
				TriggerServerEvent('tprpBot:playerDied', GetPlayerName(PlayerId()) .. ' ' .. DeathReason .. '.', Weapon)
			else
				TriggerServerEvent('tprpBot:playerDied', GetPlayerName(Killer) .. ' ' .. DeathReason .. ' ' .. GetPlayerName(PlayerId()) .. '.', Weapon)
			end
			Killer = nil
			DeathReason = nil
			DeathCauseHash = nil
