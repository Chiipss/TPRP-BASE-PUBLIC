...
AddEventHandler('playerConnecting', function()
	TriggerEvent('tprpBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n' .. GetPlayerName(source) .. ' connecting\n```', SystemAvatar, false)
end)

AddEventHandler('playerDropped', function(Reason)
	TriggerEvent('tprpBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```fix\n' .. GetPlayerName(source) .. ' left (' .. Reason .. ')\n```', SystemAvatar, false)
end)

-- On Character Load
AddEventHandler('es:playerLoaded', function( source )
	TriggerEvent('tprpBot:ToDiscord', DiscordWebhookSystemInfos, SystemName, '```css\n ID[' .. source .. '] '.. GetPlayerName(source) .. ' loaded a character.\n```', SystemAvatar, false)
end)



-- Killing Log
RegisterServerEvent('tprpBot:playerDied')
AddEventHandler('tprpBot:playerDied', function(Message, Weapon)
	...
	TriggerEvent('tprpBot:ToDiscord', DiscordWebhookKillinglogs, SystemName, Message .. ' `' .. date.day .. '.' .. date.month .. '.' .. date.year .. ' - ' .. date.hour .. ':' .. date.min .. ':' .. date.sec .. '`', SystemAvatar, false)
end)
			...
			PerformHttpRequest('http://steamcommunity.com/profiles/' .. tonumber(GetIDFromSource('steam', Source), 16) .. '/?xml=1', function(Error, Content, Head)
				...
				for i, Line in ipairs(SteamProfileSplitted) do
					if Line:find('<avatarFull>') then
						...
						TriggerEvent('tprpBot:ToDiscord', Webhook, Name .. ' [ID: ' .. Source .. ']', Message, AvatarURL, false, Source, TTS) --Sending the message to discord
						break
					end
				end
			end)
...
RegisterServerEvent('tprpBot:ToDiscord')
AddEventHandler('tprpBot:ToDiscord', function(WebHook, Name, Message, Image, External, Source, TTS)
