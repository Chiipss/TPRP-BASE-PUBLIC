...
--[[ RegisterServerEvent("tp:OpenTheDamnDoor4")
AddEventHandler("tp:OpenTheDamnDoor4", function()
    local door = vector3(252.47, 221.02, 101.8)
    local hash = 2786611474

    TriggerClientEvent('MF_PacificStandard:OpenDoor', -1, door, hash)

    print("Doors should be opened 4")

end)

RegisterServerEvent("tp:OpenTheDamnDoor5")
AddEventHandler("tp:OpenTheDamnDoor5", function()
    local door = vector3(261.12, 215.22, 101.80)
    local hash = 2786611474

    TriggerClientEvent('MF_PacificStandard:OpenDoor', -1, door, hash)

    print("Doors should be opened 5")
...
function MFP:GetIdCount(src)
    ...
    local count = xPlayer.getInventoryItem('pacificidcard')
    if count then count = count.count; end
    if count and count > 0 then 
        Citizen.Wait(100)
        xPlayer.removeInventoryItem('pacificidcard', 1)
        return count 
    else 
        return 0
    end
    ...
    Citizen.CreateThread(function(...)
        ...
        for k,v in pairs(self.LootTable[loot]) do
            if k == "money" then
                ...
                local mat = math.random(195, 2800)
                xPlayer.addAccountMoney('black_money', mat)
...
function MFP:TryLootCash(src)
    if self.BankData.LootLocs[vector3(264.24,213.72,102.50)] then
        ...
        Citizen.CreateThread(function(...)
            ...
            local amount = math.random(220000,mon)
            while tick < 15 do
                tick = tick + 1
                xPlayer.addAccountMoney('black_money', amount / 15)
                Wait(1000)
            end
            TriggerClientEvent('mythic_notify:client:SendAlert', src, { type = 'success', text = 'You managed to grab $' .. amount, length = 15000})
        end)
        return true
    else
        return false
...
RegisterServerEvent('tp:thermiteRemove')
AddEventHandler('tp:thermiteRemove',function(p,q)
	local r=ESX.GetPlayerFromId(source)r.removeInventoryItem("thermite",1)
	TriggerClientEvent('tp:thermiteEffect',-1,p,q)
end)


RegisterServerEvent('tp:thermiteSomething')
AddEventHandler('tp:thermiteSomething',function(k,l,m,n,o)
	TriggerClientEvent('tp:thermiteDoorTrigger',-1,k,l,m,n,o)
end)
...
ESX.RegisterUsableItem('thermite',function(source)
    ...
    if cops >= 4 then
        TriggerClientEvent('tp:thermiteTrigger',_source)
    else
        TriggerClientEvent("tp:notifypacific", _source)
    end
end)
