...
RegisterNetEvent('tp-sellweed:start')
AddEventHandler('tp-sellweed:start', function(pol)
	if sellingweed then
		EndSelling()
		return
	end
    ...
    while sellingweed do
		...
		if sellingweed then
			...
			local saleprice = math.random(100,120) + (pol * 10)
			local cant = math.random(5)
			if RandomNPC then
				exports['mythic_notify']:DoHudText('inform', 'A buyer is coming')
			end
			TriggerEvent("tp-sellweed:AllowSale",RandomNPC,saleprice,cant)
		end
...
RegisterNetEvent('tp-sellweed:MovePed')
AddEventHandler("tp-sellweed:MovePed",function(p)
    local usingped = p
...
RegisterNetEvent('tp-sellweed:AllowSale')
AddEventHandler('tp-sellweed:AllowSale', function(NPC,saleprice,cant)
	local timer = 0
	TriggerEvent("tp-sellweed:MovePed",NPC)
	...
	while true do
		...
		if GetDistanceBetweenCoords(GetEntityCoords(GetPlayerPed(-1)),x,y,z,true) < 10.0 then
			if cant == 1 then
					DrawText3DTest(x,y,z, "[E] Sell "..cant.." weed baggies for $" .. saleprice .. " | [H] Get out of here")
				else
					DrawText3DTest(x,y,z, "[E] Sell "..cant.." weed baggies for $" .. saleprice .. " | [H] Get out of here")
				end
...
function SellDrugs(NPC,saleprice,cant)
	...
	ESX.TriggerServerCallback('tp-sellweed:countItems', function(val)
		if val == true then
			porros = true
		...
		if sellingweed then
			...
			if math.random(50) < sellAmount then
				if not alerto then
                    ...
                    local zone = tostring(GetNameOfZone(x, y, z))
                    local Area = zoneNames[tostring(zone)]
					local msg = "There is a person in ".. Area .. " selling Weed!"
					tex = fal..": "..msg
			...
			giveAnim(NPC)
			TriggerServerEvent('tp-sellweed:payMe',cant,saleprice)
			giveAnim(GetPlayerPed(-1))
			if cant == 1 then
				exports['mythic_notify']:DoHudText('success', 'You sold '..cant..' bag $'..saleprice*cant)
			else
				exports['mythic_notify']:DoHudText('success', 'You sold '..cant..' bags $'..saleprice*cant)
			end
