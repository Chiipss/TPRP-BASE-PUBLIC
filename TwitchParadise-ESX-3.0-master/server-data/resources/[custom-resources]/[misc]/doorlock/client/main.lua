			...
			if distance < maxDistance then
				...
				if IsControlJustReleased(0, 38) then
					if isAuthorized then
						...
						TriggerServerEvent('tp_doorlock:updateState', k, doorID.locked) -- Broadcast new state of the door to everyone
					end
				end
			end
		end
...
RegisterNetEvent('tp_doorlock:setState')
AddEventHandler('tp_doorlock:setState', function(doorID, state)
	Config.DoorList[doorID].locked = state
end)