...
RegisterServerEvent('tp_doorlock:updateState')
AddEventHandler('tp_doorlock:updateState', function(doorID, state)
...
--[[ 	if not IsAuthorized(xPlayer.job.name, Config.DoorList[doorID]) then
	...
	TriggerClientEvent('tp_doorlock:setState', -1, doorID, state)
end)
