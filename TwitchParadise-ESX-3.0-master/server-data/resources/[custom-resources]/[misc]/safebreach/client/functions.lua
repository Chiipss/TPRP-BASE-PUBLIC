...
TryToCrack = function(safeEntity)
	...
	ESX.TriggerServerCallback("safebreach:checkIfSafeIsBreachable", function(state)
		if state then
			...
			exports['mythic_notify']:SendAlert('error', 'This safe is secured, Try again another time.', 15000)
		end
...
StartCrackingSafe = function(safeEntity)
	...
	exports["t0sic_loadingbar"]:StartDelayedFunction('Getting your stethoscope ready... ', 14000, function()
	local x, y, z = table.unpack(GetEntityCoords(PlayerPedId()))

	
	--TriggerServerEvent("esx_phone:send", "police", "Safe alarm gone off, breakin in progress! PLEASE RESPOND URGENTLY!!", true, { ["x"] = x, ["y"] = y, ["z"] = z })

	exports['mythic_notify']:SendAlert('success', 'Security bypassed', 7000)
			...
			if IsControlJustPressed(0, 22) then
				if dialRotation == currentSafeCorrectPosition then
					...
					if #safeLocks == 1 then
						...
						--Add police call/outlaw/take from store robberies 
						exports['mythic_notify']:SendAlert('inform', 'You triggered a silent alarm!', 5000)


						local text = "Call | Safe alarm gone off, breakin in progress! PLEASE RESPOND!"
						local coords = GetEntityCoords(GetPlayerPed(-1))
					
						TriggerServerEvent('MF_Trackables:NotifyAll',text,coords,"police",true,GetPlayerServerId(PlayerId()))
					...
					if #safeLocks < Config.TotalLocks then
						...
						ESX.TriggerServerCallback("safebreach:safeBreached", function(breached)
							if breached then
								...
								exports['mythic_notify']:SendAlert('success', 'You cracked the safe!', 15000)
							else
								exports['mythic_notify']:SendAlert('inform', 'The safe is already open!', 15000)
							end
						end, {
							["store"] = currentStreetName,
							["doorCoords"] = {
					...
					exports['mythic_notify']:SendAlert('error', 'Wrong combination, Try again.', 15000)
				end
...
TakeSafeItem = function(itemEntity, safeData)
	...
	if DoesEntityExist(itemEntity) then
		...
		exports['mythic_notify']:SendAlert('error', 'The safe is empty, Someone else must have allready been here.', 15000)
	end
end
