...
function MFF:Awake(...)
  while not ESX do Citizen.Wait(0); end
      self:DSP(true)
      self.dS = true
	  print("FleecaRobberyStarted: Started")
      self:Start()
end
...
function MFF:RewardPlayer(data,id)
  ...
  if id then
    ...
    for k,v in pairs(self.LootTable) do
      ...
      if amount > 0 then
        local matty = math.random(150, 480)
        xPlayer.addMoney(matty) 
...
function MFF:NotifyPolice(data)
	...
	--[[ TriggerEvent('tprp:GetJobPlayers', 'police', function(players)
		for k, v in pairs(players) do
			TriggerClientEvent('MF_Fleeca:NotifyPolice', k, data)
		end
	end) ]]
