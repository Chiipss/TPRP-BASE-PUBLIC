...
function ProcessImpoundment(plate, current_time, vehicles)
  for index, vehicle in pairs(vehicles) do
    ...
    MySQL.Async.execute("INSERT INTO `impounded_vehicles` (`plate`, `vehicle`, `owner`, `impounded_at`, `finance`, `financetimer`, `retailprice`, `job`) VALUES(@plate, @vehicle, @owner, @timestamp, @finance, @financetimer, @retailprice, @job)", {
      ['@plate'] = plate,
      ['@vehicle'] = vehicle.vehicle,
      ['@owner'] = vehicle.owner,
      ['@timestamp'] = current_time,
      ['@finance'] = vehicle.finance,
      ['@financetimer'] = vehicle.financetimer,
...
function RetrieveVehicle(plate)
  ...
  MySQL.Async.fetchAll('SELECT * FROM impounded_vehicles WHERE plate = @plate LIMIT 1', {
    ...
    for index, vehicle in pairs(vehicles) do
      ...
      if Config.OwnedVehiclesHasPlateColumn then
        MySQL.Async.execute("INSERT INTO `owned_vehicles` (`plate`, `vehicle`, `owner`, `finance`, `financetimer`, `retailprice`, `state`, `job`) VALUES(@plate, @vehicle, @owner, @finance, @financetimer, @retailprice, '0', @job)",
          {
            ['@plate'] = plate,
            ['@vehicle'] = vehicle.vehicle,
            ['@owner'] = vehicle.owner,
