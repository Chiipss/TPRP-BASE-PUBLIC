...
function DoDropOff()
	...
	if DoesEntityExist(deliveryPed) and not IsEntityDead(deliveryPed) then
		...
		if success then
			...
			Citizen.Wait(2000)
			exports['mythic_notify']:SendAlert('success', 'The delivery was on point, your pager will be updated with the next drop off', 10000)
		else
			exports['mythic_notify']:SendAlert('error', 'The drop off failed', 10000)
		end
...
AddEventHandler("oxydelivery:client", function()
	...
	while tasking do
		...
		if dstcheck < 40.0 and not pedCreated and (oxyVehicle == veh or dstcheck2 < 15.0) then
			...
			DeleteCreatedPed()
			CreateOxyPed()
			exports['mythic_notify']:SendAlert('inform', 'You are close to the drop off', 10000)
		end
		toolong = toolong - 1
		if toolong < 0 then
			...
			tasking = false
			OxyRun = false
			exports['mythic_notify']:SendAlert('inform', 'You are no longer selling Oxy due to taking too long to drop off', 10000)
		end
    ...
    while true do
		...
		elseif OxyRun then
			...
			if not DoesEntityExist(oxyVehicle) or GetVehicleEngineHealth(oxyVehicle) < 200.0 or GetVehicleBodyHealth(oxyVehicle) < 200.0 then
				OxyRun = false
				tasking = false
				exports['mythic_notify']:SendAlert('error', 'The dealer isnt giving you anymore locations due to the state of his car', 10000)
			else
				if tasking then
			        Citizen.Wait(30000)
			    else
					...
					if job ~= "police" then
				    	...
				    	if IsControlJustReleased(0,38) then
							...
							if daytime > 17 or daytime < 9 then
								exports['mythic_notify']:SendAlert('error', 'Its too late - noones buying shit!', 10000)
		            		else
...
AddEventHandler("oxydelivery:startDealing", function()
	...
	OxyRun = true
	firstdeal = true 
	exports['mythic_notify']:SendAlert('inform', 'A car has been left outside for you. Your pager will be updated with locations soon', 10000)
	CreateOxyVehicle()
end)