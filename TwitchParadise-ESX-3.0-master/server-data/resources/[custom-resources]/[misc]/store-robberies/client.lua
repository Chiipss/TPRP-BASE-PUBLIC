...
RegisterNetEvent('loffe_robbery:onPedDeathSafe')
AddEventHandler('loffe_robbery:onPedDeath', function(store)
    SetEntityHealth(peds[store], 0)
    TriggerEvent("tp:storeRobbed")
end)
...
AddEventHandler('loffe_robbery:robberyOver', function()
    robbing = false
    TriggerEvent("tp:storeRobbed")
    --[[ local gruppe = math.random(1,10)
    if gruppe <= 3 then
        TriggerServerEvent("tp_gruppe:akceptujto")
        exports['mythic_notify']:SendAlert('inform', 'The clerk just gave you some information!', 7000)
    end ]]
end)
