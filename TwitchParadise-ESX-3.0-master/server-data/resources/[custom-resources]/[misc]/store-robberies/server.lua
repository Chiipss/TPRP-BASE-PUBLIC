...
AddEventHandler('loffe_robbery:pedDead', function(store)
    if not deadPeds[store] then
        ...
        if math <= 3 then
            TriggerClientEvent('loffe_robbery:onPedDeathSafe', -1, store)
            local second = 1000
...
ESX.RegisterServerCallback('loffe_robbery:canRob', function(source, cb, store)
	...
	--[[ TriggerEvent('tprp:GetJobCount', 'police', function(count)
	    if count >= Config.Shops[store].cops then
			cb(Config.Shops[store].robbed and not deadPeds[store])
		else
			cb('no_cops')
		end
	end) ]]
...
AddEventHandler('loffe_robbery:rob', function(store)
	...
	--[[ TriggerEvent('tprp:GetJobPlayers', 'police', function(players)
		for k, v in pairs(players) do
			TriggerClientEvent('loffe_robbery:msgPolice', k, store, src)
		end
	end) ]]
