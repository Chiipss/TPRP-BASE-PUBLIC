...
AddEventHandler("denoms", function(denomsp)
  ...
  hasdenoms = denomsp
  if hasdenoms then
    TriggerServerEvent("tp:addStatusChat", "Your currency has Multiple Denominations")
  else
    TriggerServerEvent("tp:addStatusChat", "Your currency no longer has Multiple Denominations")
  end
end)
...
AddEventHandler("Evidence:StateSet",function(stateId,stateLength)
    if Currentstates[stateId]["timer"] < 10 and stateLength ~= 0 then
        TriggerServerEvent("tp:addStatusChat", Currentstates[stateId]["text"])
...
function myBleeds()
    ...
    if not initialbleed --[[ and math.random(100) < 20 ]] then
        if lightbleed or heavybleed or lightestbleed then
            TriggerEvent("DoLongHudText","You notice blood oozing from your body the more you move (NP 1).",2)
    ...
    if initialbleed then
        TriggerEvent("DoLongHudText","You notice blood oozing from your body the more you move (NP 2).",2)
        --TriggerEvent("randomBoneDamage")
        for i = 1, 7 do
             if lightbleed or heavybleed or lightestbleed then
                SetEntityHealth(PlayerPedId(), GetEntityHealth(PlayerPedId()) - bleedmulti)
...
--[[ RegisterCommand("check",function(source, args)
        ...
        boneIssuesTarget(bt,target)
		--TriggerEvent("Evidence:CurrentDamageListTarget",source,target)
end,false)
