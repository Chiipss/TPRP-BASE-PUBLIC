...
AddEventHandler("route68_kasyno:WymienZetony", function(count)
    ...
    if xPlayer then
        ...
        if xItem.count < count then
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'You don`t have that many chips!', length = 5000})
        elseif xItem.count >= count then
            ...
            if kwota > societyAccount.money then
                TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'The casino can not pay you out at this time, please come back later.', length = 5000})
            else
                xPlayer.removeInventoryItem('zetony', count)
                xPlayer.addMoney(kwota)
                societyAccount.removeMoney(kwota)
                TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = 'You got $'..kwota..' for '..count..' chips.', length = 5000})
            end
        end
    end
end)
...
AddEventHandler("route68_kasyno:KupZetony", function(count)
    ...
    if xPlayer then
        ...
        local kwota = math.floor(count * 5)
        if kwota > cash then
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'error', text = 'You don`t have that much money!', length = 5000})
        elseif kwota <= cash then
            ...
            TriggerClientEvent('mythic_notify:client:SendAlert', _source, { type = 'success', text = 'You got '..count..' chips for $'..kwota..'.', length = 5000})
        end
    end
end)
