...
--[[
RegisterNetEvent('tp_repair:repair')
AddEventHandler('tp_repair:repair', function()
	local playerPed = PlayerPedId() 
	local vehicle   = ESX.Game.GetVehicleInDirection()
	local coords    = GetEntityCoords(playerPed)

	if IsPedSittingInAnyVehicle(playerPed) then
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'error', text = 'You cna tdo this while in the vehicle.', length = 7000})
		return
	end
		...
		if DoesEntityExist(vehicle) then
			...
			TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'Vehicle repaired.', length = 7000})
			isBusy = false
		end)
		
	else
		TriggerClientEvent('mythic_notify:client:SendAlert', source, { type = 'success', text = 'No vehicle near-by.', length = 7000})
	end
end)
--]]
