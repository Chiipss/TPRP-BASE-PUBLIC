...
Citizen.CreateThread(function()
    ...
    while true do
        ...
        if pedInVeh or locationAlwaysOn then
            ...
            drawTxt(locationText, 4, locationColorText, 0.5, screenPosX + 0.02, screenPosY + 0.075)
            ...
            if pedInVeh and GetIsVehicleEngineRunning(vehicle) and vehicleClass ~= 13 then
                ...
                if not mph then
                    ...
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.8, screenPosX + 0.000, screenPosY + 0.048)
                    drawTxt("KPH", 2, speedColorText, 0.4, screenPosX + 0.011, screenPosY + 0.048)
                else
                    -- Get vehicle speed in MPH and draw speedometer
                    local speed = currSpeed*2.23694
                    local speedColor = (speed >= speedLimit) and speedColorOver or speedColorUnder
                    drawTxt(("%.3d"):format(math.ceil(speed)), 2, speedColor, 0.8, screenPosX + 0.000, screenPosY + 0.048)
                    drawTxt("MPH", 2, speedColorText, 0.4, screenPosX + 0.011, screenPosY + 0.048)
                end
                
                -- Draw fuel gauge
                local fuelColor = (currentFuel >= fuelWarnLimit) and fuelColorOver or fuelColorUnder
                drawTxt(("%.3d"):format(math.ceil(currentFuel)), 2, fuelColor, 0.8, screenPosX + 0.030, screenPosY + 0.048)
                drawTxt("FUEL", 2, fuelColorText, 0.4, screenPosX + 0.041, screenPosY + 0.048)

                -- Draw cruise control status
                local cruiseColor = cruiseIsOn and cruiseColorOn or cruiseColorOff
                drawTxt("CRUISE", 2, cruiseColor, 0.4, screenPosX + 0.030, screenPosY + 0.018)
                ...
                if vehicleClass ~= 8 then
                    ...
                    drawTxt("SEATBELT", 2, seatbeltColor, 0.4, screenPosX + 0.061, screenPosY + 0.048)
                end
            end
        end
    end
...
compass.show = true
compass.position = {x = 0.176, y = 0.968, centered = true}
compass.width = 0.02
compass.fov = 80
