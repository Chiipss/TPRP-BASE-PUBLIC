      ...
      function (result)
        if result[1] ~= nil then
          ...
          if count > 0 then
            ...
            MySQL.Async.fetchAll('SELECT owner FROM disc_inventory WHERE owner = @oldplate', {
                ['@oldplate'] = oldplate,},
                function (result)
                    if result[1] ~= nil then
                        MySQL.Async.execute('UPDATE disc_inventory SET owner = @newplate WHERE owner = @oldplate',{
                            ['@oldplate'] = oldplate,
                            ['@newplate'] = newplate
                        })
                    end
