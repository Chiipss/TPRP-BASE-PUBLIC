...
MySQL.ready(function()
	...
	for i=1, #result2 do
		if ESX.Jobs[result2[i].job_name] then
			...
			print(('tprp_base: invalid job "%s" from table job_grades ignored!'):format(result2[i].job_name))
		end
	end
	...
	for k,v in pairs(ESX.Jobs) do
		if next(v.grades) == nil then
			ESX.Jobs[v.name] = nil
			print(('tprp_base: ignoring job "%s" due to missing job grades!'):format(v.name))
		end
	end
end)
...
AddEventHandler('esx:playerLoaded', function(source)
	...
	-- damien change - test
	TriggerEvent('tprp:PlayerLoaded', xPlayer, source)
end)
