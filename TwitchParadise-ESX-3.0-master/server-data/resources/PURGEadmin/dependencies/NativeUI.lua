...
function UIMenuItem.New(Text, Description, sprite)
    if sprite == nil then
        sprite = 0
    else 
        sprite = 21
    ...
    _UIMenuItem = {
        ...
        LeftBadge = { Sprite = Sprite.New("commonmenu", "", 0, 0, 40, 40), Badge = sprite},
...
function MenuPool:AddSubMenu(Menu, Text, Description, KeepPosition, KeepBanner, TxtDictionary, TxtName, Label, Badge)
    if Menu() == "UIMenu" then
        local Item = UIMenuItem.New(tostring(Text), Description or "")
        Menu:AddItem(Item)
        if label ~= nil then Item:SetRightLabel(label) elseif Badge ~= nil then Item:SetRightBadge(Badge) else Item:RightLabel("→→→") end
        local SubMenu
        if KeepPosition then
            SubMenu = UIMenu.New(Menu.Title:Text(), Text, Menu.Position.X, Menu.Position.Y, TxtDictionary, TxtName)
        else
...
function NativeUI.CreateLockedItem(Text, Description)
    return UIMenuItem.New("~m~"..Text, Description, 21)
end
